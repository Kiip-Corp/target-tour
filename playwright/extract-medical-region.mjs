 ...
 *
 * 진료과목별 비율은 국가 자료(4번)와 달리 월 구분이 없다. 지역 탭(tabDiv=4)이 등록하는 차트가
 * 6종뿐이고 그 안에 「진료과목별 월별 비율 추이」가 없기 때문이다 — 국가 탭(tabDiv=3)은 8종이라
 * 월별 추이가 따로 나온다. 지역 쪽은 조회 기간 전체를 뭉갠 스냅샷 하나뿐이라
