...
같은 사이트의 [고부가관광 > 의료관광](https://datalab.visitkorea.or.kr/datalab/portal/theme/getMedicalTourSearch.do)
＞ "국가별 의료소비 추이" 탭을 도는 별도 매크로다. 국가 선택이 노출되는 유일한 탭이라
`tabDiv=3` 으로 고정하고, 조합마다 zip 1개(CSV 8종)를 받는다.

```bash
node playwright/datalab-medical.mjs                    # 6개국 × 2020~2026
node playwright/datalab-medical.mjs --nations 대만,일본
node playwright/datalab-medical.mjs --from-year 2024 --to-year 2024
node playwright/extract-medical.mjs                    # zip → data/4국가월간/의료소비_월별.csv
```
...
data/4국가월간/
  의료소비_월별.csv   # 국가명,기준연월,의료 소비액,의료 소비건수,전체 외국인 대비 소비액/소비건수 비율(%)
  진료과목_월별.csv   # 국가명,기준연월,진료과목,소비액 비율(%),소비건수 비율(%)
```

진료과목은 "비율 추이"(월별) 쪽만 뽑는다 — 기간 전체를 뭉갠 스냅샷은 월별을 합치면 나온다.

`전체 외국인 대비 비율(%)`은 국적 무관 분모를 역산하는 데 쓴다. `국가 소비액 ÷ 비율 × 100` 은
6개국 어디서 계산해도 같은 값이 나오고, 5번(지역별)의 "전국" 연간 합계와도 맞는다 —
4번과 5번이 같은 지표·같은 단위라는 뜻이다. 보드는 이 성질로 5번 연간 파일의 최신 연도가
몇 월까지 누적된 값인지 판별한다.

## 지역별 의료소비 추이 (5번)

같은 의료관광 페이지의 "지역별 의료소비 추이" 탭을 도는 매크로다. 지역 선택이 노출되는 유일한
탭이라 그 탭으로 고정하고, 조합마다 zip 1개(CSV 6종)를 받는다.

```bash
node playwright/datalab-medical-region.mjs                                  # 전국 + 시도 18개 × 2020~2026
node playwright/datalab-medical-region.mjs --regions 제주특별자치도,강원특별자치도
node playwright/extract-medical-region.mjs                                  # zip → data/5지역월간/의료소비_월별.csv
```

이미 받은 zip은 건너뛰므로, 중간에 끊기면 같은 명령을 다시 돌리거나 남은 지역만 `--regions`로
지정하면 이어서 받는다.

```
playwright/getMedicalTourSearch/
  manifest.jsonl
  서울특별시/
    202001-202012.zip
    ...
...
data/5지역월간/
  의료소비_월별.csv   # 시도명,기준연월,의료 소비액,의료 소비건수
  진료과목_연간.csv   # 시도명,기준연도,진료과목,소비액 비율(%),소비건수 비율(%)
```

진료과목은 국가 자료(4번)와 달리 월 구분이 없다 — 조회 기간 전체를 뭉갠 스냅샷 하나뿐이라
기간(=연도) 단위로만 뽑는다.

추출한 CSV에는 17개 시도 외에 두 행 묶음이 더 있다.

- **전국** — 17개 시도 합계보다 11~20% 크다. 시도가 특정되지 않은 소비가 들어 있어서다.
  보드는 이 값을 분모로 삼아 "지도·목록 합계가 전국의 몇 %인지"를 밝힌다.
- **전남광주통합특별시** — 광주광역시 + 전라남도와 원 단위까지 같은 **중복 행**이다. 합계를 낼
  때 반드시 빼야 한다. 2026.07부터는 구 시도가 제공되지 않아 그 달만 통합시로 나오므로,
  보드는 통합시를 빼는 대신 광주·전남을 2026년 7월 결측으로 둔다.

인기 소비 동네 CSV는 `/popularNeighborhoods` 가 따로 다뤄 뽑지 않는다.
