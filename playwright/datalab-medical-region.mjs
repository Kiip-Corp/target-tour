/**
 * 한국관광 데이터랩 - 고부가관광(의료관광) > 지역별 의료소비 추이 일괄 다운로드 매크로
 *
 *   https://datalab.visitkorea.or.kr/datalab/portal/theme/getMedicalTourSearch.do
 *
 * 조회 조건
 *   탭    : "지역별 의료소비 추이" (지역 선택이 노출되는 유일한 탭)
 *   지역  : --regions (기본: 충청북도부터 시작해 전국 + 광역시/도 18개 전부)
 ...
 *
 * 사용법
 *   node playwright/datalab-medical-region.mjs
 *   node playwright/datalab-medical-region.mjs --regions 충청북도,전국
 *   node playwright/datalab-medical-region.mjs --from-year 2024 --to-year 2024
 *   node playwright/datalab-medical-region.mjs --dry-run
 *   node playwright/datalab-medical-region.mjs --headed --slow 150
 */
...
/** "지역별 의료소비 추이" 탭. tabDiv=4 일 때만 지역 선택(#sido_select_box)이 노출된다. */
const TAB = { index: '4', link: '#tab4 > a', name: '지역별 의료소비 추이' };

/** tabDiv=4 조회 1회당 발생하는 getTempleteData.do 호출 수 (초기 진입 시엔 tab0 차트 2개가 더 붙는다) */
const EXPECT_CHARTS = 6;

/** "전체 다운로드"(tabDiv=4)가 그대로 재사용하는 차트 파라미터 키 — CSV 6개에 대응 */
const REQUIRED_CHART_KEYS = ['chart_16', 'chart_12', 'chart_13', 'chart_17', 'chart_14', 'chart_15'];

/** 기본 순회 순서: 충청북도부터, 나머지는 팝업 목록 순서 */
const FIRST_REGION = '충청북도';
...
function parseArgs(argv) {
  const out = {
    regions: null, // null = 팝업 목록 전체(전국 + 광역시/도 18개)
    fromYear: 2020,
    toYear: 2026,
    outDir: path.join(HERE, 'getMedicalTourSearch'),
    headed: false,
    slow: 0,
    dryRun: false,
    force: false,
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    const next = () => argv[++i];
    switch (a) {
      case '--regions': out.regions = next().split(',').map(s => s.trim()).filter(Boolean); break;
...
/** "지역별 의료소비 추이" 탭으로 전환. 탭 핸들러는 실제로 탭이 바뀔 때만 funSrch() 를 호출한다. */
async function openRegionTab(page, state) {
  ...
  await page.waitForSelector('#sido_select_box', { state: 'visible' });
...
/** 지역 팝업에서 광역시/도를 고르고 확인. popSet() 이 곧바로 funSrch() 를 호출한다. */
async function selectRegion(page, region) {
  await dismissOverlay(page);
  await page.click('#area-select');
  await page.waitForSelector('#popup1', { state: 'visible' });
  await page.locator(`#srchNatCdList1 a[onclick*="'${region.cd}'"]`).first().click();
  await page.click('#popup1 .modal-foot a:has-text("확인")');
  await page.waitForSelector('#popup1', { state: 'hidden' });
  await dismissOverlay(page);
  const code = await page.inputValue('#sggCd');
  const name = await page.inputValue('#sggNm');
  if (name !== region.nm || code !== region.cd) {
    throw new Error(`지역 선택 실패: expected ${region.nm}(${region.cd}), got ${name}(${code})`);
  }
}
 ...
 */
async function runQuery(page, state, { region, from, to, timeout, alreadyQueried = false }) {
  ...
  await page.waitForFunction(
    ({ cd, from, to, keys }) => {
      ...
      const c = reg.chart_16;
      return c && c.SGG_CD === cd && c.BASE_YM1 === from && c.BASE_YM2 === to;
    },
    { cd: region.cd, from, to, keys: REQUIRED_CHART_KEYS },
    { timeout: 60_000 },
  ).catch(() => {
  ...
  const title = (await page.textContent('#chart_16_title')).trim();
  if (!title.includes(region.nm)) log(`  ! 차트 제목이 "${title}" (기대 ${region.nm})`);
...
async function main() {
  ...
  if (!args.dryRun && (!id || !pw)) {
    console.error(
      ...
      '  조회 동작만 확인하려면: node playwright/datalab-medical-region.mjs --dry-run',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    await openRegionTab(page, state);
    log(`탭: ${TAB.name}`);

    // 지역 목록 (팝업의 버튼에서 코드/이름 추출). 전국('00') + 광역시/도 18개.
    await dismissOverlay(page);
    await page.click('#area-select');
    await page.waitForSelector('#popup1', { state: 'visible' });
    const allRegions = await page.$$eval('#srchNatCdList1 a', els =>
      els.map(a => {
        const m = /funChangeSido\(\s*'(\d+)'\s*,\s*'([^']+)'/.exec(a.getAttribute('onclick') || '');
    ...
    let regions;
    if (args.regions) {
      regions = args.regions.map(nm => {
        const hit = allRegions.find(r => r.nm === nm);
        if (!hit) throw new Error(`지역 "${nm}" 없음. 가능: ${allRegions.map(r => r.nm).join(', ')}`);
        return hit;
      });
    } else {
      // 충청북도부터 시작하고 나머지는 목록 순서 그대로
      const first = allRegions.filter(r => r.nm === FIRST_REGION);
      regions = [...first, ...allRegions.filter(r => r.nm !== FIRST_REGION)];
    }
    log(`지역 ${regions.length}개 × ${years.length}년 = ${regions.length * years.length}건`);
    ...
    for (const region of regions) {
      ...
      const dir = path.join(args.outDir, safe(region.nm));
      ...
      for (const year of years) {
        ...
        if (from > to) {
          log(`SKIP ${region.nm} ${year}: 제공 기간(${minBaseYm}~${maxBaseYm}) 밖`);
          summary.skipped++;
          await record({ region: region.nm, year, status: 'skipped', reason: `제공 기간 ${minBaseYm}~${maxBaseYm} 밖` });
          continue;
        }
        ...
        if (!args.force && !args.dryRun) {
          ...
          if (found) {
            summary.skipped++;
            log(`SKIP ${region.nm} ${range} (이미 있음: ${found})`);
            continue;
          }
        }

        const label = `${region.nm} ${range}`;
        for (let attempt = 1; attempt <= args.retries + 1; attempt++) {
          ...
          try {
            ...
            // 지역을 처음 고를 때는 popSet() 이 곧바로 funSrch() 를 호출하므로 조회 버튼을 누르지 않는다.
            ...
            await runQuery(page, state, { region, from, to, timeout: args.timeout, alreadyQueried });

            if (args.dryRun) {
              log(`  (dry-run) 조회 완료 · 차트응답 ${state.chartResponses}`);
              await record({ region: region.nm, range, status: 'dry-run' });
              summary.ok++;
              break;
            }
            ...
            await record({
              region: region.nm, range, status: 'ok',
            ...
            if (lastAttempt) {
              summary.failed++;
              log(`  ✗ ${label}: ${e.message}`);
              await record({ region: region.nm, range, status: 'failed', error: e.message });
            } else {
