...
export default function MarketingBoard({
  ...
  /**
   * 선택 방문지의 의료소비 추이(「지역별 의료소비」 = 데이터랩 자료 5번). 「국가별 의료소비」(4번)와 축이 다르다 —
  ...
  return (
    <div style={{ fontFamily: "ui-monospace, monospace", fontSize: 13 }}>
      ...
      <Panel
        ...
        note={`방문·관광소비·의료소비(지역)는 ${regionName} 기준이고, 의료소비(국가)는 ${country} 기준이되 지역 구분이 없어 전국 합계입니다 — 「국가별 의료소비」는 국가축, 「지역별 의료소비」는 지역축이라 둘이 겹치지 않습니다. 단위가 달라(방문 = 명, 소비 3종 = 천원) 각 지표의 최대월을 100으로 지수화했습니다 — 값의 크기가 아니라 '몰리는 시점'을 비교하는 차트입니다.`}
      >
        <MultiLineChart
