...
import { BASE, COAST_PATH, px, py } from "../medicalConsumptionMap/koreaGeo";

export type RegionRow = { region: string; short: string; lat: number; lng: number; visit: number; spend: number };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(260px,1fr) 220px", gap: 16 }}>
        <div style={{ position: "relative", border: `1px solid ${BORDER}`, borderRadius: 10, padding: 8 }}>
          <svg viewBox={`0 0 ${BASE.w} ${BASE.h}`} style={{ width: "100%", height: "auto", display: "block" }}>
            ...
            {rows.map((r) => {
              const v = r[metric];
              const rad = Math.max(4, Math.sqrt(v / maxValue) * 34);
              ...
              return (
                <g key={r.short} onClick={() => setSelected(r.short)} style={{ cursor: "pointer" }}>
                  ...
                  >
                    <title>{`${r.region} · ${metric === "visit" ? "방문자" : "관광소비"} 비율 ${fmtVal(v)}%`}</title>
                  </circle>
                </g>
              );
            })}
            ...
            {rows.slice(0, 6).map((r) => (
              <text
                key={`t-${r.short}`}
                x={px(r.lng)}
                y={py(r.lat) - Math.max(4, Math.sqrt(r[metric] / maxValue) * 34) - 4}
                textAnchor="middle"
                fontSize={9.5}
                fontWeight={700}
                fill={INK}
          ...
          >
            <div style={{ fontSize: 9, color: MUTED, letterSpacing: "0.08em", marginBottom: 5 }}>
              {metric === "visit" ? "방문자 비율" : "관광소비 비율"} (원 크기 · 색)
            </div>
            <div
              style={{
                width: 100,
        ...
        <div style={{ border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 11, color: MUTED, letterSpacing: "0.06em", marginBottom: 8 }}>
            {current?.country} · {metric === "visit" ? "방문자 비율" : "관광소비 비율"} 순위
          </div>
          {rows.map((r, i) => (
            <div
              key={r.short}
