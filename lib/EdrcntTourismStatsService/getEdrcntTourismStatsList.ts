/* ────────────────────────────────────────────────────────────────
   ...
   callback URL: http://openapi.tour.go.kr/openapi/service/EdrcntTourismStatsService/getEdrcntTourismStatsList
   ...
   | NAT_CD | 국가코드 | 6 | 0 | 112 | 국가코드(예: 100=한국) |
   ...
   | edCd | 출/입국 구분코드 | 2 | 1 | E | D=국민해외관광객, E=방한외래관광객 |
...
export type EdrcntTourismStatsParams = {
  YM: string; // 년월 YYYYMM (필수)
  NAT_CD?: string; // 국가코드
  ED_CD?: "D" | "E"; // D=국민해외관광객, E=방한외래관광객
  pageNo?: number;
  numOfRows?: number;
};

export type EdrcntTourismStatsItem = {
  ed: string;
  edCd: string;
  natCd: string;
  natKorNm: string;
...
type ApiResponse = {
  response: {
    ...
    body?: {
      items?: { item?: EdrcntTourismStatsItem[] | EdrcntTourismStatsItem } | "";
      numOfRows: number;
      pageNo: number;
      totalCount: number;
    };
...
export async function fetchEdrcntTourismStatsList(
  params: EdrcntTourismStatsParams
): Promise<EdrcntTourismStatsItem[]> {
  ...
  if (params.ED_CD) query.set("ED_CD", params.ED_CD);

  const res = await fetch(`${ENDPOINT}?${query.toString()}`);
  if (!res.ok) {
    throw new Error(`출입국관광통계 API 요청 실패: ${res.status} ${res.statusText}`);
  }

  const json = (await res.json()) as ApiResponse;
  const header = json.response?.header;
  if (!header) {
    throw new Error("출입국관광통계 API 응답 형식이 올바르지 않습니다.");
  }
  if (String(header.resultCode) !== "0000") {
    throw new Error(`출입국관광통계 API 오류(${header.resultCode}): ${header.resultMsg}`);
  }
