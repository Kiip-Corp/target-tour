/* ────────────────────────────────────────────────────────────────
   한국관광공사 TourAPI(data.go.kr) · AreaTarDivService/areaExpDivList
   지역별 관광 소비 다양성(Expenditure Diversity) 정보 목록 조회

   공통 안내(CORS, 환경변수, 서비스 명세)는 ./client.ts 참고.

   지역별 관광 소비 다양성 정보 목록을 조회하는 기능입니다.
   관광 소비 다양성(Expenditure Diversity): 연령별 소비액 세부 지표로 구성

   callback URL: https://apis.data.go.kr/B551011/AreaTarDivService/areaExpDivList
   ...
   | expDivIxCd | 관광 소비 다양성 지표 코드 | 0 | 3204 | 32: 전체, 3201: 10대 소비액, 3202: 20대 소비액, 3203: 30대 소비액, 3204: 40대 소비액, 3205: 50대 소비액, 3206: 60대 소비액, 3207: 70대 소비액 |
   ...
   | expDivIxCd | 관광 소비 다양성 지표 코드 | 0 | 3204 | 32: 전체, 3201: 10대 소비액, 3202: 20대 소비액, 3203: 30대 소비액, 3204: 40대 소비액, 3205: 50대 소비액, 3206: 60대 소비액, 3207: 70대 소비액 |
   | expDivIxNm | 관광 소비 다양성 세부 지표명 | 0 | 40대 소비액 | 관광객 다양성 세부 지표명 |
   | expDivIxVal | 관광 소비 다양성 세부 지표값 | 0 | 72 | 관광객 다양성 세부 지표값 |
   ──────────────────────────────────────────────────────────────── */

import { fetchKtoList, resolveAreaCodes, type AreaCodeParams } from "./client";

export type AreaExpDivListParams = AreaCodeParams & {
  baseYm: string; // 조회 기준월 YYYYMM
  expDivIxCd?: string; // 관광 소비 다양성 지표 코드
  pageNo?: number;
  numOfRows?: number;
};

export type AreaExpDivListItem = {
  baseYm: string;
  areaCd: string;
  areaNm: string;
  signguCd: string;
...
export async function fetchAreaExpDivList(
  params: AreaExpDivListParams
): Promise<AreaExpDivListItem[]> {
  ...
  if (params.expDivIxCd) query.expDivIxCd = params.expDivIxCd;

  return fetchKtoList<AreaExpDivListItem>("AreaTarDivService", "areaExpDivList", query);
}
